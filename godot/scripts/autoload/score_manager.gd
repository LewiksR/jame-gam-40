...
func _get_total_score() -> int:
	var total: int = 0
	for score in _individual_scores.values():
		total += score
	return total


func _increment_score(score_name: String, score: int):
	_individual_scores[score_name] += score
	Events.score_changed.emit(_get_total_score())
...
func _on_game_started():
	for key in _individual_scores.keys():
		_individual_scores[key] = 0
